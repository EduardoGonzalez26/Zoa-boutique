...
> **Última actualización:** 2026-09-18 (sesión: remediación de seguridad de `.tmp-qa`, fix del chip de cupón en la bolsa, hardening `vipCode` y `npm run lint` en 0).
> **Rama:** `main` ↔ `origin/main`; el commit `c1c7c27` («Add: codigo secreto») fue **reescrito en esta sesión** (`git reset --soft HEAD~1` + `git rm -r --cached .tmp-qa` + commit + force-push con lease) para expulsar los 676 archivos de `.tmp-qa/`: el hash vigente es el que muestra `git log` (no reutilizar `c1c7c27` como HEAD). Working tree **con cambios sin commitear** al cierre de esta actualización (fix de cupón, hardening, lint y estos docs; ver `git status`). Detalle y acciones manuales pendientes en **§7.1**.
...
**Verificación de la sesión (2026-09-18, código secreto «ENTREAMIGAS»):** `npx tsc --noEmit` ✅ exit 0; `npm run build` ✅ exit 0 (151 páginas, ISR 1m/1y); la feature no aportaba errores de lint. Estado commiteado originalmente en `c1c7c27`, reescrito el mismo día para expulsar `.tmp-qa/` (hash vigente según `git log`).

**Cierre de sesión (2026-09-18):** `npm run lint` ✅ en 0 (corregidos los 6 errores preexistentes y `.tmp-qa/**` ignorado en ESLint y git); fix del chip de cupón en `components/CartDrawer.tsx` (se renderiza con el `vipCode` persistido, la X de quitar cupón aparece tras recargar; sigue habiendo un solo cupón a la vez, anti-stacking) y `vipCode` null-safe en `app/api/webhooks/payment-success/route.ts`. Cambios en working tree **sin commitear** al cierre (ver `git status`).
...
|---|---|---|---|
| — (working tree) | 2026-09-18 | Fix: chip de cupón persistido + hardening `vipCode` | 2 archivos **sin commitear** (ver `git status`): `components/CartDrawer.tsx` (el chip ya no depende del `couponStatus` efímero; se renderiza con el `vipCode` persistido, así la X de quitar cupón aparece tras recargar; un solo cupón a la vez, anti-stacking) y `app/api/webhooks/payment-success/route.ts` (`vipCode?: string` + `(vipCode ?? "")` en `buildCustomerEmail`, `vipCode = ""` por defecto al desestructurar el body del endpoint público). |
| — (working tree) | 2026-09-18 | Lint: 0 errores + ignore de `.tmp-qa` | 6 archivos **sin commitear** (ver `git status`): `lib/skydropx.ts` y `app/api/debug-skydropx/route.ts` (`no-explicit-any`), `app/api/process-payment/route.ts` (`as any`), `app/api/reviews/route.ts` (`baseUrl` sin uso), `eslint.config.mjs` (`globalIgnores` con `.tmp-qa/**`) y `.gitignore` (`.tmp-qa/`). `npm run lint` ✅ en 0. |
...
1. **`.tmp-qa/` — REMEDIADO (2026-09-18).** Los 676 archivos (675 de `chrome-profile/` + `chrome.log`) con artefactos sensibles del perfil Chrome (Cookies, Login Data, History, Session Storage) ya **no están en el árbol de `main`**: `c1c7c27` fue reescrito localmente (`git reset --soft HEAD~1` + `git rm -r --cached .tmp-qa` + commit + force-push con lease) y el hash vigente es el de `git log`. `.tmp-qa/` queda ignorado en git (`.gitignore`) y en ESLint (`globalIgnores`). Quedan 2 acciones **manuales** no automatizables: (a) purgar los objetos colgantes de GitHub vía soporte **solo si el repo es público** (el force-push no los elimina en servidores de terceros); (b) **rotar credenciales/sesiones** de las cuentas usadas en ese perfil (Google, MercadoPago, Vercel, GitHub, Skydropx, Resend) por precaución. No volver a commitear `.tmp-qa/`.
...
7. **Verificación:** ejecutar `tsc`/`build`/QA de hidratación («0 warnings», §8.6 MASTER) tras cualquier cambio; última referencia verde: build 2026-09-18 con «ENTREAMIGAS» (151 páginas).
8. **«Lo más vendido» — RESUELTO:** commiteada y pusheada junto con el código secreto (ADR-07 y ADR-08) en `c1c7c27`, hoy reescrito el 2026-09-18; hash vigente en `git log`.
...
11. **Lint — RESUELTO (2026-09-18):** corregidos los 6 errores preexistentes (`no-explicit-any` en `app/api/debug-skydropx/route.ts`, `app/api/process-payment/route.ts` y `lib/skydropx.ts`; `baseUrl` sin uso en `app/api/reviews/route.ts`) y `.tmp-qa/**` se ignora en `eslint.config.mjs` (`globalIgnores`) y en `.gitignore`. `npm run lint` ✅ en 0 (working tree, sin commitear al cierre).
12. **`vipCode` null-safe en `payment-success` — RESUELTO (2026-09-18):** `vipCode?: string` + `(vipCode ?? "").trim()` en `buildCustomerEmail` y `vipCode = ""` por defecto al desestructurar el body de `app/api/webhooks/payment-success/route.ts` (endpoint público). Working tree, sin commitear al cierre (ver `git status`).
